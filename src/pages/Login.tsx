...
export default function Login({ isAdminMode = false }: LoginProps) {
  ...
  return (
    <div className="flex items-center justify-center min-h-[70vh] animate-in fade-in duration-500">
      <Card className="w-full max-w-md border-2 border-primary/20 shadow-2xl backdrop-blur-sm bg-card/90">
        <CardHeader className="text-center space-y-1 pb-8">
          ...
          <CardTitle className="text-4xl font-black italic uppercase tracking-tighter">
            {isAdminMode ? 'Admin Entrance' : 'Elite Access'}
          </CardTitle>
          <CardDescription className="text-base font-bold text-muted-foreground/80 uppercase tracking-widest">
            {isAdminMode ? 'Authorised Personnel Only' : 'Discord Verification Required'}
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-6">
          {isAdminMode ? (
            <div className="space-y-4">
              <Button 
                className="w-full h-14 gap-3 text-lg font-bold uppercase italic shadow-lg hover:scale-[1.01] transition-transform"
              ...
              <Button 
                ...
                className="w-full gap-2 text-muted-foreground hover:text-foreground italic font-medium"
              >
                <ArrowLeft className="w-4 h-4" />
                Switch to Elite Access
              </Button>
            </div>
          ) : (
            <div className="space-y-4">
              <Button 
                className="w-full h-16 gap-4 text-xl font-black uppercase italic shadow-xl shadow-[#5865F2]/20 transition-all bg-[#5865F2] hover:bg-[#4752C4] text-white" 
              ...
              <div className="pt-4 border-t border-muted-foreground/10 text-center">
                <Button 
                  ...
                  className="text-muted-foreground/50 hover:text-primary text-xs uppercase tracking-widest font-mono"
                >
                  Admin Portal Access
                </Button>
              </div>
